...
def send_command(motor_id, direction, speed, distance):
    cmd = f"{motor_id},{direction},{speed:.2f},{distance:.2f}\n"
    ser.write(cmd.encode())
    print(f"[전송] ID:{motor_id}, DIR:{direction}, SPEED:{speed:.2f}, DIST:{distance:.2f}")
    time.sleep(0.05)

def move(path, r_start, theta_start, linear_speed=0, duration=0, visualize=False):
    ...
    for r, theta in path:
        ...
        send_command(1, dir_r, abs(speed_r), abs(dr))
